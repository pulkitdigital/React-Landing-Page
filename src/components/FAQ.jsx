...
import React, { useState, useRef, useLayoutEffect, useState as useReactState } from "react";
import { motion, AnimatePresence } from "framer-motion";
...
function SmoothAnswer({ open, children }) {
  const ref = useRef(null);
  const [height, setHeight] = useReactState(0);
  ...
  return (
    ...
    >
      <div
        ref={ref}
        className="px-4 sm:px-6 pb-3 sm:pb-4 text-[15px] sm:text-base text-gray-600 bg-white/90 rounded-2xl shadow-sm"
      >
        {children}
      </div>
    </motion.div>
...
export default function FAQSection() {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[850px] sm:max-w-[900px] lg:max-w-[1000px] xl:max-w-[1100px] bg-white/80 rounded-2xl shadow-lg p-1.5 sm:p-2">
        {questions.map((item, idx) => {
          const open = openIdx === idx;
          return (
            <motion.div
            ...
            >
              ...
              {/* ✅ Smooth Answer */}
              <SmoothAnswer open={open}>{item.a}</SmoothAnswer>
            </motion.div>
          );
        })}
      </div>
