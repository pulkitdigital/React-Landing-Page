...
function FeatureCard({ title, desc, img, delay = 0 }) {
    return (
        <motion.div
            ...
            className="relative rounded-[18px] border border-black/5 shadow-sm overflow-hidden bg-white/80 backdrop-blur"
            //   style={{
            //     background: `linear-gradient(135deg, ${BLUE}22 0%, #ffffff 26%, #ffffff 74%, ${ORANGE}22 100%)`,
            //   }}
            style={{
                background: `linear-gradient(120deg, ${BLUE}33, #ffffff, ${ORANGE}33)`,
            }}
        >
            <div className="rounded-[16px] bg-white/75 p-5 sm:p-6">
    ...
    return (
        <section className="w-full" style={{ backgroundColor: GRID_BG }}>
            <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8 py-16">
                ...
                {/* 🌈 Full-gradient wrapper around the grid */}
                <div
                    className="rounded-2xl p-3 sm:p-4"

                >
                    {/* Grid with tighter gaps, and column offset */}
                    ...
                    >
                        ...
                        {/* Right column — more top margin on md+ to drop it lower */}
                        <div className="flex flex-col gap-4 lg:gap-5 md:mt-10 lg:mt-12">
                            <FeatureCard {...cards[1]} delay={0.18} />
                            {/* small upward tweak for the bottom-right card if you want that “staggered” feel */}
